package com.bgasparotto.sparkscala.job.graphx
...
object GraphX {
  ...
  def main(args: Array[String]) {
    ...
    val names = sc.textFile("dataset/marvel-graph/Marvel-names.txt")
    val verts = names.flatMap(parseNames)

    // Build up our edges
    val lines = sc.textFile("dataset/marvel-graph/Marvel-graph.txt")
    ...
    val initialGraph = graph.mapVertices(
      (id, _) => if (id == root) 0.0 else Double.PositiveInfinity
    )

    // Now the Pregel magic
    val bfs = initialGraph
      .pregel(Double.PositiveInfinity, 10)(
